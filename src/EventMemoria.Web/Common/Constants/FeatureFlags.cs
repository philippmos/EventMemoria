namespace EventMemoria.Web.Common.Constants;

public static class FeatureFlags
{
    public const string EnableUploadPage = nameof(EnableUploadPage);
    public const string EnableSubscribePage = nameof(EnableSubscribePage);
    public const string EnableQrCodePage = nameof(EnableQrCodePage);
    public const string EnablePhotoUpload = nameof(EnablePhotoUpload);
    public const string EnablePhotoDownload = nameof(EnablePhotoDownload);
    public const string EnableGalleryPage = nameof(EnableGalleryPage);
    public const string ShowAuthorInfo = nameof(ShowAuthorInfo);
    public const string EnableVideoUpload = nameof(EnableVideoUpload);
    public const string ShowAvatar = nameof(ShowAvatar);
    
}
